...
- benchmark和普通的单元测试用例一样， 都位于_test.go 文件中。

- 函数名以 Benchmark 开头， 参数是 b *testing.B。和普通的单元测试用例很像， 单元测试函数名以 Test开头，参数是 t *testing.T
...
benchmark 用例的参数 `b *testing.B`， 有个属性`b.N`表示这个需要运行的次数。 `b.N` 对于每个用例都是不一样的。

那这个值是如何决定的呢？ `b.N` 从1开始， 如果该用例能够在1s 内完成， `b.N` 的值会增加，再次执行。 `b.N` 的值大概以1、2、3、5、10、20、30、50、100这样的序列递增， 越到后面，增加的越快。我们仔细观察上述的输出：
